...
let cms = {
    ...
    'desktop': [
        ['<i class="bi bi-arrow-clockwise"></i> Refresh', `$('#desktop').css('opacity','0');setTimeout(()=>{$('#desktop').css('opacity','1');},100);`],
        ['<i class="bi bi-circle-square"></i> Toggle Theme', 'toggletheme()'],
        `<a onmousedown="window.open('https://github.com/','_blank');" win12_title="https://github.com/" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)"><i class="bi bi-github"></i> Creator Github</a>`, ['<i class="bi bi-info-circle"></i> Galaxy', `$('#win-about>.about').addClass('show');$('#win-about>.update').removeClass('show');openapp('about');if($('.window.about').hasClass('min'))minwin('about');`],
    ],
    'winx': [
        function(arg) {
        ...
        '<hr>', ['<i class="bi bi-gear"></i> 设置', `openapp('setting')`],
        ...
        '<hr>', ['<i class="bi bi-power"></i> 关机', `window.location='shutdown.html'`],
...
dps = {
    'notepad.file': [
        ...
        '<hr>', ['<i class="bi bi-x"></i> 退出', `isOnDp=false;hidedp(true);hidewin('notepad')`],
    ],
    'notepad.edit': [
        ...
        '<hr>', ['<i class="bi bi-arrow-return-left"></i> 撤销 <info>Ctrl+Z</info>', 'document.execCommand(\'undo\')'],
    ...
    'notepad.view': [
        ...
        '<hr>', ['<i class="bi bi-fonts"></i> 字体', 'hidedp(true);showwin(\'notepad-fonts\');apps.notepadFonts.reset();'],
    ]
}
let dpt = null,
    isOnDp = false;
...
let nts = {
    'about': {
        cnt: `<p class="tit">Galaxy</p>
            <p>It's a beggining</p>`,
        btn: [{
                type: 'main',
                text: 'Understood',
                js: 'closenotice();'
            },
            {
                type: 'detail',
                text: 'Read More',
                js: "closenotice();openapp('about');if($('.window.about').hasClass('min'))minwin('about');$('.dock.about').removeClass('show')"
            },
        ]
    },
    'feedback': {
...
let apps = {
    ...
    whiteboard: {
        ...
        load: () => {
            ...
            apps.whiteboard.windowResizeObserver.observe($('.window.whiteboard')[0], {
                box: 'border-box'
            });
        },
        resize: () => {
        ...
        draw: (e) => {
            let offsetX, offsetY, left = $('#win-whiteboard')[0].getBoundingClientRect().left,
                top = $('#win-whiteboard')[0].getBoundingClientRect().top;
            ...
            if (localStorage.getItem('camera')) {
                ...
                apps.camera.video.addEventListener('canplay', () => {
                    if (!apps.camera.streaming) {
                        ...
                        apps.camera.windowResizeObserver.observe($('.window.camera')[0], {
                            box: 'border-box'
                        });
                    ...
                    folder: {
                        'Program Files': {
                            ...
                            file: [{
                                    name: 'about.exe',
                                    ico: 'icon/about.svg',
                                    command: "openapp('about')"
                                },
                                {
                                    name: 'setting.exe',
                                    ico: 'icon/setting.svg',
                                    command: "openapp('setting')"
                                },
                            ]
                        },
                        'Windows': {
...
function hide_widgets() {
    ...
    setTimeout(() => {
        $('#widgets').removeClass('show-begin');
    }, 200);
}
